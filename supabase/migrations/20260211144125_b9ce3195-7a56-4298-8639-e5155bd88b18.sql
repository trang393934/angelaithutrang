...
-- RPC function to aggregate user management data for admin dashboard
...
RETURNS TABLE(
  ...
  camly_lifetime_earned BIGINT,
  camly_lifetime_spent BIGINT,
  fun_money_received BIGINT,
  gift_internal_sent BIGINT,
  gift_internal_received BIGINT,
  gift_web3_sent BIGINT,
  gift_web3_received BIGINT,
  total_withdrawn BIGINT,
  withdrawal_count BIGINT,
  pplp_action_count BIGINT,
  pplp_minted_count BIGINT,
  wallet_address TEXT
)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT
    ...
    wd.last_wallet AS wallet_address
  FROM profiles p
