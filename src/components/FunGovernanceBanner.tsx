...
interface FunGovernanceBannerProps {
  className?: string;
}

export const FunGovernanceBanner = ({ className = "" }: FunGovernanceBannerProps) => {
  const { t } = useLanguage();
  ...
  return (
    <motion.div
      ...
      className={className}
    >
      <Link to="/docs/fun-governance">
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-amber-600 via-yellow-600 to-orange-600 p-6 shadow-xl shadow-amber-500/25 hover:shadow-2xl hover:shadow-amber-500/40 transition-all duration-500 group cursor-pointer">
          {/* Background effects */}
          ...
          <div className="relative flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-amber-100 text-xs font-medium uppercase tracking-wider">{t("governance.bannerTag")}</span>
              </div>
              <h3 className="text-lg font-bold text-white truncate">
                FUN GOVERNANCE
              </h3>
              <p className="text-amber-100/80 text-sm truncate">
                {t("governance.bannerDesc")}
              </p>
            </div>
